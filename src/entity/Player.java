...
public class Player extends Entity {
	...
	public void update() {
		...
		if (keyH.upPressed == true || keyH.downPressed == true || keyH.leftPressed == true || keyH.rightPressed == true) { // only updates player sprite if key is pressed. So sprite isn't always moving when idle.
			...
			// sprite image updater
			spriteCounter++;
			if (spriteCounter > 12) { // image changes every 10 frames (since update method is called 60 times per second)
				if (spriteNum == 1) {
					spriteNum = 2;
				}
				else if (spriteNum == 2) {
...
// not using rectangle as player character anymore:
